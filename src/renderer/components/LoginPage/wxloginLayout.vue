    ...
    <a-layout-content style="padding:0px 50px;">
      ...
      >
        <a-col style="height:100%">
          <div
            style="height:80%;display: flex;flex-direction: column;justify-content: center; text-align:center"
          >
            <a-row
              style="height:100%"
              class="login-text"
            ...
            >
              <a-col style="height:100%">
                <img
                  v-if="imageReady"
                  :src="imageUrl"
                  style="width: auto; height: auto; max-width: 100%; max-height: 100%; border-radius:50%;"
                  alt=""
                />
              </a-col>
            </a-row>
          ...
          <div
            class="login-text"
            style="text-align:center;margin-top:15px;height:20%"
            v-if="!codeLoading"
          >
            {{ tipMsg }}
          </div>
...
export default {
  ...
  methods: {
    getUserInfoAndSwitch(token) {
      ...
      ipcRenderer.send("setStorageSync", "UserToken", token);
      this.$http
        .get("/user/userInfo", {})
    ...
    toPassword: function() {
      this.$matomo &&
        this.$matomo.trackEvent("Buttons", "Click", "switchToUsernameLogin");
